...
public class UserServiceImplTest {
    ...
    public void testGetProfilePictureOrDefaultCaseDefault(){
        Mockito.when(profilePictureDao.findById(Mockito.anyLong())).thenReturn(Optional.empty());
    ...
    public void testGetProfilePictureOrDefaultCaseNonDefault(){
        byte[] img = new byte[]{0};
        Mockito.when(profilePictureDao.findById(Mockito.anyLong())).thenReturn(Optional.of(new ProfilePicture(1, img)));
